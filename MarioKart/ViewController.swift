...
class ViewController: UIViewController,
  ...
  // Exercise 3: Decipher the mystery box and apply the correct effect on mario
    private func decipher(mysteryBox: MysteryBox) {
        ...
        guard let effectDictionary = mysteryBox.mysteryEffect as? [String: String] else { assertionFailure("Expecting value of type dictionary")
            return }
        guard let effect = effectDictionary["effect"] else { assertionFailure("Expecting value of type String")
            return }
        if effect == "translate" {
